...
- `projects/pricepilot/index.html` - product page for the Vercel deployment.
- `projects/pricepilot/server.js` - Express app serving the landing page, discovery routes, health check, and paid `/find_cheapest` endpoint.
- `projects/pricepilot/dual402.js` - payment middleware that accepts both x402 and MPP on the same route.
- `projects/pricepilot/api/index.js` - Vercel serverless entrypoint.
- `projects/pricepilot/tools/find_cheapest.py` - Hermes CLI path for Nimble-backed product lookup.
- `projects/pricepilot/tools/find_cheapest_stub.js` - Vercel demo resolver with the same response contract as the paid endpoint.
- `projects/pricepilot/integrations/nimble_client.py` - retailer scraping/search integration.
- `projects/pricepilot/integrations/clickhouse_client.py` - price event and tracking storage.
- `projects/pricepilot/integrations/senso_client.py` - report-generation interface used by alert flows.
- `projects/pricepilot/skills/` - Hermes skills for best-price search, URL checks, tracking, history, and price alerts.
- `projects/pricepilot/hermes/` - setup, config, validation, and runtime scripts for the Telegram assistant.
...
- `projects/pricepilot/tools/find_cheapest_stub.js` keeps the Vercel paid route testable and marketplace-indexable while the live Nimble resolver is being wired into that route.
...
The Hermes assistant was run from the original hackathon workspace layout where PricePilot lived at `projects/pricepilot`. The scripts in `projects/pricepilot/hermes/` document that workflow and may need path adjustments if this repo is cloned standalone.
